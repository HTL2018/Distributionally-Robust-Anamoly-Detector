...
dr_alpha = 40; % From Previous calculation
...
for i=2:N
    ...
    z(:,i-1)  =  r(:,i-1)'*inv(Sigma)*r(:,i-1);
end


chi_squared_false_alarm = sum(z>alpha)/N
dr_false_alarm          = sum(z>dr_alpha)/N

dr_alpha = 40;

figure;
% Plot the DR threshold Plane
pointA=[-10 15 dr_alpha];
pointB=[10 -15 dr_alpha];
pointC=[10 15 dr_alpha];
normal = cross(pointA-pointB, pointA-pointC); %# Calculate plane normal
syms x y zt
P = [x,y,zt];
planefunction = dot(normal, P-pointA);
zplane = solve(planefunction, zt);
h(1) = fmesh(zplane, [-20, 20, -20, 20],'EdgeColor','blue');
hold on;
% Plot the Chi Squared threshold Plane
pointA=[-10 15 alpha];
pointB=[10 -15 alpha];
pointC=[10 15 alpha];
normal = cross(pointA-pointB, pointA-pointC); %# Calculate plane normal
